...
public enum ErrorCode {
    ...
    MAINTENANCE_WINDOW_NOT_FOUND("Không tìm thấy lịch bảo trì", 404),
    INVALID_MAINTENANCE_TIME_RANGE("Thời gian bảo trì không hợp lệ", 400),
    MAINTENANCE_WINDOW_OVERLAPS("Thời gian này trùng với một lịch bảo trì khác", 409),
    MAINTENANCE_WINDOW_ALREADY_CANCELLED("Lịch bảo trì này đã được hủy", 400),
    MAINTENANCE_WINDOW_CANNOT_BE_CREATED_IN_PAST("Không thể tạo lịch bảo trì trong quá khứ", 400),
    PLANNED_MAINTENANCE_HAS_AFFECTED_SHOWTIMES("Còn suất chiếu bị ảnh hưởng; hãy điều phối các suất này trước", 409),
    MAINTENANCE_WINDOW_NOT_ACTIVE("Lịch bảo trì này không còn hiệu lực", 409),
    MAINTENANCE_WINDOW_CANNOT_BE_CANCELLED_AFTER_START("Bảo trì đã bắt đầu; hãy dùng thao tác 'Phòng hoạt động trở lại'", 409),
    MAINTENANCE_WINDOW_CANNOT_BE_RESOLVED_BEFORE_START("Lịch bảo trì chưa bắt đầu nên chưa thể xác nhận phòng hoạt động trở lại", 409),
    MAINTENANCE_EXTENSION_MUST_INCREASE_END_TIME("Thời gian kết thúc mới phải muộn hơn thời gian hiện tại", 400),
    ...
    AUTO_SCHEDULE_PREVIEW_STALE("Auto schedule preview is stale and must be regenerated", 409),
    ...
    AUTO_SCHEDULE_PREFLIGHT_BLOCKED("Auto schedule preflight found blocking configuration issues", 422),
    AUTO_SCHEDULE_NO_FEASIBLE_CANDIDATES("Auto schedule has no feasible candidates", 422),
    AUTO_SCHEDULE_SOLVER_INFEASIBLE("Demand-aware optimizer found no feasible schedule", 422),
    AUTO_SCHEDULE_SOLVER_TIMEOUT("Demand-aware optimizer timed out before finding a feasible schedule", 503),
    AUTO_SCHEDULE_SOLVER_MODEL_INVALID("Demand-aware optimizer model is invalid", 500),
