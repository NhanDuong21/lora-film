...
public class AuthServiceImpl implements AuthService {
	...
	public RegistrationInitiatedResponse register(RegisterRequest request) {
		try {
			...
			try {
				...
				throw new com.project.authservice.exception.BusinessException("Internal error processing registration", e);
			}
	...
	public void verify(VerifyRequest request) {
		...
		try {
			...
			throw new com.project.authservice.exception.BusinessException("Internal error", e);
		}
		...
		try {
			...
			log.error("Failed to publish USER_PROFILE_CREATED event via outbox for accountId={}", savedAccount.getId(), kafkaEx);
			throw new com.project.authservice.exception.BusinessException("System overload. Failed to send profile creation request. Please try again later.", kafkaEx);
		}
	...
	public void resetPassword(com.project.authservice.dto.request.ResetPasswordRequest request) {
		...
		com.project.authservice.entity.PasswordResetToken tokenEntity = passwordResetTokenRepository.findByToken(request.getToken())
				.orElseThrow(() -> new com.project.authservice.exception.BusinessException("Invalid token"));
				
		if (tokenEntity.getUsed() || tokenEntity.getExpiresAt().isBefore(LocalDateTime.now())) {
			throw new com.project.authservice.exception.BusinessException("Token expired or already used");
		}
	...
	@Override
	public com.project.authservice.dto.response.RegistrationStatusResponse getRegistrationStatus(String requestId) {
		throw new UnsupportedOperationException("Not implemented yet");
	}

	@Override
	public void completeValidation(String requestId, com.project.authservice.dto.ValidationResult result) {
		throw new UnsupportedOperationException("Not implemented yet");
	}
}
