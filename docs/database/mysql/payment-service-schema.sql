...
--
-- Refunds are stored as a dedicated aggregate. A successful capture remains
-- SUCCESS; refund state and refundable balance are derived from
...
-- PURPOSE:
--   Durable full/partial refund aggregate. Automatic full refunds cover late
--   provider success, Booking confirmation rejection, duplicate capture and a
--   cancelled Showtime. Partial refunds are limited to concessions, price
--   differences and operational adjustments. Per-ticket refunds are
--   intentionally not represented until BookingTicket owns that lifecycle.
-- ============================================================================
CREATE TABLE `payment_refunds` (
  ...
  CONSTRAINT `uk_payment_refunds_request`
  ...
  CONSTRAINT `chk_payment_refunds_lease`
      CHECK (
        (`locked_at` IS NULL AND `locked_until` IS NULL AND `locked_by` IS NULL)
        OR
        (`locked_at` IS NOT NULL AND `locked_until` IS NOT NULL
