CREATE TABLE `notification_templates` (
  ...
  `content` TEXT NOT NULL COMMENT 'Noi dung mau co chua cac bien cho',
...
CREATE TABLE `notification_logs` (
  ...
  `recipient` VARCHAR(150) NULL COMMENT 'Email, phone number, device token. NULL chi duoc phep voi IN_APP',
  `channel_type` VARCHAR(30) NOT NULL COMMENT 'EMAIL, SMS, PUSH_NOTIFICATION, IN_APP',
  `actual_title` VARCHAR(255) NULL COMMENT 'Tieu de thuc te sau khi render template',
  `actual_content` TEXT NOT NULL COMMENT 'Noi dung thuc te sau khi render template',
...
ALTER TABLE `notification_logs`
ADD CONSTRAINT `uk_notification_logs_idempotency_key`
