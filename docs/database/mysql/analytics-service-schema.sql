...
ON UPDATE CURRENT_TIMESTAMP COMMENT 'Lan cap nhat gan nhat'
);
...
UNIQUE KEY `uk_movie_daily_revenue_movie_date`
(`movie_id`, `stat_date`),

INDEX `idx_movie_daily_revenue_stat_date`
(`stat_date`)
);
...
`processed_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Thoi diem event duoc aggregate thanh cong',

INDEX `idx_processed_analytics_events_type`
(`event_type`),

INDEX `idx_processed_analytics_events_processed_at`
(`processed_at`)
);
