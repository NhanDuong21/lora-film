...
Schema được bootstrap thủ công từ file canonical
[`promotion-service-schema.sql`](../database/mysql/promotion-service-schema.sql)
trước khi start service. Hibernate chạy ở chế độ `ddl-auto=validate` và không tự
tạo hoặc thay đổi bảng.
...
references as `VARCHAR(36)` trong schema canonical. `publicId` values generated by
