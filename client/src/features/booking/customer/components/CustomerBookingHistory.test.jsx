  ...
  content: [
    {
      ...
      presentation: {
        ...
        seats: [
          {
            seatPublicId: "seat-a1",
            label: "A1",
            type: "STANDARD",
            price: 85000,
          },
          {
            seatPublicId: "seat-a2",
            label: "A2",
            type: "STANDARD",
            price: 85000,
          },
...
describe("CustomerBookingHistory pending recovery actions", () => {
  ...
  it("offers checkout and cancellation only for a live pending booking", async () => {
    ...
    expect(
      screen.getAllByRole("button", { name: /hủy giữ ghế/i }),
    ).toHaveLength(1);
    expect(
      screen.getByText(/thời gian giữ ghế đã kết thúc/i),
    ).toBeInTheDocument();
    expect(screen.getByText("Mưa đỏ")).toBeInTheDocument();
    expect(
      screen.getByText(/LoraFilm Sense City Cần Thơ · Phòng 3/),
    ).toBeInTheDocument();
  ...
  it("presents each history filter with a clear accessible label", async () => {
    ...
    expect(
      screen.getByLabelText(/Tìm theo mã đơn hoặc tên phim/i),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: /Đổi chiều sắp xếp/i }),
    ).toHaveTextContent("Mới nhất");
  });
});
