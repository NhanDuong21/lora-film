...
export default function EmployeeRefundRequestPage() {
  ...
  const changeType = refundType => setForm(value => ({
    ...value,
    refundType,
    refundComponent: refundType === 'FULL' ? 'FULL_ORDER' : 'CONCESSION',
    amount: '',
  }));
  ...
  const submit = event => {
    ...
    if (form.refundType === 'PARTIAL' && (!Number(form.amount) || Number(form.amount) > Number(payment.refundableAmount))) {
      setNotice({ tone: 'info', title: 'Số tiền chưa hợp lệ', message: `Số tiền phải lớn hơn 0 và không vượt quá ${money(payment.refundableAmount, payment.currency)}.` });
      return;
    }
    run(async () => {
      const payload = {
      ...
      {!payment ? (
        ...
        <form onSubmit={submit} className="grid items-start gap-6 lg:grid-cols-[360px_1fr]">
          <aside className="space-y-4 rounded-3xl border border-zinc-800 bg-zinc-900 p-6 lg:sticky lg:top-6">
            ...
            <dl className="space-y-3 text-sm"><div className="flex justify-between gap-3"><dt className="text-zinc-500">Phim</dt><dd className="max-w-48 text-right font-bold text-zinc-200">{payment.movieTitle || 'Đơn đặt vé'}</dd></div><div className="flex justify-between gap-3"><dt className="text-zinc-500">Đã thanh toán</dt><dd className="font-black text-zinc-100">{money(payment.amount, payment.currency)}</dd></div><div className="flex justify-between gap-3"><dt className="text-zinc-500">Đã hoàn trước đó</dt><dd className="font-bold text-zinc-300">{money(payment.refundedAmount, payment.currency)}</dd></div></dl>
          ...
          <section className="space-y-6 rounded-3xl border border-zinc-800 bg-zinc-900 p-6">
            ...
            <div><label className="text-sm font-black text-zinc-100">2. Hạng mục hoàn tiền *</label><div className="mt-3 grid gap-3 sm:grid-cols-2">{COMPONENTS.filter(item => form.refundType === 'FULL' ? item.value === 'FULL_ORDER' : item.value !== 'FULL_ORDER').map(item => <button key={item.value} type="button" onClick={() => setForm(value => ({ ...value, refundComponent: item.value }))} className={`rounded-xl border p-4 text-left ${form.refundComponent === item.value ? 'border-amber-500/50 bg-amber-500/10' : 'border-zinc-700 bg-zinc-950/40'}`}><span className="text-sm font-black text-zinc-200">{item.label}</span><span className="mt-1 block text-xs leading-5 text-zinc-500">{item.hint}</span></button>)}</div></div>

            {form.refundType === 'PARTIAL' ? <label className="block text-sm font-black text-zinc-100">3. Số tiền đề nghị hoàn *<div className="relative mt-2"><CircleDollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500" size={19} /><input required type="number" min="1" max={Number(payment.refundableAmount)} step="1000" value={form.amount} onChange={event => setForm(value => ({ ...value, amount: event.target.value }))} placeholder={`Tối đa ${money(payment.refundableAmount, payment.currency)}`} className="h-12 w-full rounded-xl border border-zinc-700 bg-zinc-950 pl-12 pr-4 text-sm outline-none focus:border-amber-500" /></div></label> : null}
