...
const resolveCustomerProfileTab = (tab, fallback) =>
  customerProfileTabs.has(tab) ? tab : fallback;
...
export default function CustomerProfileView({
  ...
  useEffect(() => {
    ...
    setActiveTab(
      resolveCustomerProfileTab(searchParams.get("tab"), initialTab),
    );
  }, [initialTab, location]);
  ...
  const membershipProgress = useMemo(() => {
    ...
    const targetMin =
      scoreData?.nextTier?.minAccumulatedPoints || currentMin + 400;
    if (targetMin === currentMin) return 100;
    const currentProgress = Math.max(
      0,
      (scoreData?.accumulatedPoints || 0) - currentMin,
    );
    return Math.min(
      100,
      Math.max(
        0,
        Math.round((currentProgress / (targetMin - currentMin)) * 100),
      ),
    );
  }, [scoreData]);
  ...
  const handleSavePassword = async () => {
    ...
    if (
      ...
      showErrorNotice(
        "Mật khẩu mới cần ít nhất 8 ký tự, chữ hoa, chữ thường, số và ký tự đặc biệt.",
      );
      return;
    }
          ...
          {profileLoading ? (
            ...
            <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl text-center space-y-4 max-w-xl mx-auto my-12 shadow-2xl animate-in fade-in zoom-in-95 duration-300">
              ...
              <p className="text-xs text-zinc-400 leading-relaxed">
                Hồ sơ thành viên của bạn đang được hệ thống thiết lập và đồng
                bộ. Vui lòng nhấn nút tải lại bên dưới sau vài giây.
              </p>
              <button
                type="button"
                onClick={async () => {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-4 pb-6 border-b border-zinc-800/80">
                    <div className="relative w-16 h-16 shrink-0 rounded-full border-2 border-brand-orange overflow-hidden bg-zinc-950 group">
                      {hasImageSource(avatarUrl) ? (
                        ...
                        <User
                          className="h-full w-full p-4 text-zinc-600"
                  ...
                  <div className="pt-6 space-y-4">
                    ...
                    <div className="flex justify-between items-center text-[9px] text-zinc-500 font-bold pt-1.5">
                      ...
                      <div className="text-center">
                        <span
                          className={`block ${totalSpending >= 2000000 ? "text-brand-orange" : ""}`}
                        >
                          2.000.000đ
                        </span>
                      ...
                      <div className="text-right">
                        <span
                          className={`block ${totalSpending >= 4000000 ? "text-brand-yellow" : ""}`}
                        >
                          4.000.000đ
                        </span>
                        <span>Vàng (Gold)</span>
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 space-y-4 shadow-xl">
                  <span className="text-zinc-500 text-[10px] font-black uppercase tracking-wider block border-b border-zinc-800/80 pb-2">
                    HỖ TRỢ THÀNH VIÊN
                  </span>
                          ...
                          <div className="flex gap-4">
                            <button
                              ...
                              className={`flex-grow py-3 rounded-xl border text-xs font-bold transition-all duration-300 bg-zinc-900/50 text-zinc-500 border-zinc-800 cursor-not-allowed ${
                                gender === "MALE"
                                  ? "opacity-100 border-brand-orange/40 text-brand-orange"
                                  : "opacity-40"
                              }`}
                            >
                              Nam
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <p className="text-zinc-500 text-[10px]">
                          Danh sách các vé và dịch vụ đã mua trực tuyến hoặc tại
                          quầy
                        </p>
                      </div>
                  ...
                  {activeTab === "gifts" && (
                    <div className="space-y-6">
                      <div>
                        ...
                        <p className="text-zinc-500 text-[10px]">
                          Danh sách voucher và quà tặng đang có hiệu lực sử dụng
                        </p>
                      </div>

                      <CustomerPromotionCenterPage embedded />
                    </div>
                  )}
                  ...
                  {activeTab === "policy" && (
                    <div className="space-y-6">
                      <div>
                        ...
                        <p className="text-zinc-500 text-[10px]">
                          Quy định tích lũy điểm thưởng và thăng hạng thành viên
                        </p>
                      </div>
                ...
                <div className="flex justify-center">
                  <div className="w-24 h-24 rounded-full border border-zinc-700 overflow-hidden bg-zinc-950">
                    {hasImageSource(tempAvatarUrl) ? (
                      ...
                      <User
                        className="h-full w-full p-6 text-zinc-600"
                        aria-label="Chưa có ảnh xem trước"
                      />
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      setAvatarFile(file);
                      setTempAvatarUrl(
                        file ? URL.createObjectURL(file) : avatarUrl,
                      );
                    }}
