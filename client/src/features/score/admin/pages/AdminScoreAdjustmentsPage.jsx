...
import { AlertTriangle, ArrowLeft, MinusCircle, PlusCircle, RotateCcw, Search } from 'lucide-react';
...
export default function AdminScoreAdjustmentsPage() {
  ...
  const {
    userScore, userHistory, isLoadingUserScore, errorUserScore, fetchUserScore,
  ...
  const submitLookup = event => { event.preventDefault(); load(accountId.trim()); };
  ...
  const submitAdjust = async event => {
    ...
    const approved = await confirm({
      ...
      message: `${adjust.type === 'ADD' ? 'Cộng' : 'Trừ'} ${n(points)} điểm cho Account ID ${accountId}. ${adjust.affectAccumulatedPoints ? 'Điểm hạng và hạng thành viên cũng có thể thay đổi.' : 'Điểm hạng không thay đổi.'} Case: ${adjust.caseId}.`,
  ...
  const submitReverse = async event => {
    ...
    const approved = await confirm({ title: 'Đảo giao dịch điều chỉnh', message: `Đảo History ID ${reverse.historyId} trên Account ID ${accountId}. Hệ thống chỉ cho đảo một lần và sẽ ghi audit theo ${reverse.caseId}.`, confirmLabel: 'Đảo giao dịch', tone: 'danger' });
    if (!approved) return;
    try {
  ...
  return (
    <section className="mx-auto max-w-6xl space-y-6 text-white">
      <header className="rounded-3xl border border-white/10 bg-zinc-900/50 p-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-orange">Complaint operations</p><h1 className="mt-2 text-2xl font-black">Xử lý khiếu nại điểm</h1><p className="mt-2 text-sm text-zinc-400">Mọi thay đổi yêu cầu case, lý do, idempotency key và xác nhận tác động trước khi ghi ledger.</p></div>{userScore ? <Link to={`/admin/scores/viewer?accountId=${accountId}`} className="inline-flex items-center gap-2 text-xs font-black text-zinc-400 hover:text-white"><ArrowLeft size={15} /> Về hồ sơ điểm</Link> : null}</div></header>

      <article className="rounded-3xl border border-white/10 bg-zinc-900/40 p-5">
        <form onSubmit={submitLookup} className="flex flex-col gap-2 sm:flex-row"><label className="relative flex-1"><Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-600" size={18} /><input value={accountId} onChange={event => setAccountId(event.target.value)} inputMode="numeric" aria-label="Account ID" placeholder="Nhập Account ID" className="h-11 w-full rounded-xl border border-white/10 bg-black/30 pl-11 pr-4 text-sm outline-none focus:border-brand-orange" /></label><button disabled={isLoadingUserScore || !accountId} className="h-11 rounded-xl bg-brand-orange px-5 text-xs font-black text-black disabled:opacity-50">Tải hồ sơ</button></form>
        {errorUserScore && !userScore ? <p className="mt-3 text-xs text-red-400">{errorUserScore}</p> : null}
        {userScore ? <div className="mt-5 grid gap-3 sm:grid-cols-4"><div className="rounded-xl bg-black/30 p-3"><p className="text-[10px] uppercase text-zinc-600">Khả dụng</p><p className="mt-1 text-xl font-black text-emerald-400">{n(available)}</p></div><div className="rounded-xl bg-black/30 p-3"><p className="text-[10px] uppercase text-zinc-600">Tạm giữ</p><p className="mt-1 text-xl font-black text-sky-400">{n(userScore.heldPoints)}</p></div><div className="rounded-xl bg-black/30 p-3"><p className="text-[10px] uppercase text-zinc-600">Điểm hạng</p><p className="mt-1 text-xl font-black">{n(userScore.accumulatedPoints)}</p></div><div className="rounded-xl bg-black/30 p-3"><p className="text-[10px] uppercase text-zinc-600">Trạng thái điểm</p><p className="mt-1 text-sm font-black">{userScore.status}</p></div></div> : null}
      </article>
      ...
      {userScore ? <>
        ...
        {tab === 'adjust' ? <form onSubmit={submitAdjust} className="space-y-5 rounded-3xl border border-white/10 bg-zinc-900/40 p-6">
          ...
          <label className="block text-xs font-bold text-zinc-400">Lý do và căn cứ xử lý *<textarea required minLength={5} maxLength={255} rows={3} value={adjust.reason} onChange={event => setAdjust(v => ({ ...v, reason: event.target.value }))} placeholder="Mô tả booking/thanh toán/refund liên quan và kết luận xác minh…" className="mt-2 w-full rounded-xl border border-white/10 bg-black/30 p-4 text-sm text-white outline-none focus:border-brand-orange" /></label>
        ...
        <article className="overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/40"><div className="border-b border-white/10 p-5"><h2 className="font-black">Giao dịch gần đây</h2><p className="mt-1 text-xs text-zinc-500">Chọn nhanh History ID khi cần đảo một điều chỉnh thủ công.</p></div>{history.length ? <div className="overflow-x-auto"><table className="min-w-full text-left text-xs"><thead className="bg-white/[0.025] text-[10px] uppercase text-zinc-600"><tr><th className="px-5 py-3">History ID</th><th className="px-5 py-3">Loại</th><th className="px-5 py-3">Biến động</th><th className="px-5 py-3">Case / lý do</th><th className="px-5 py-3"></th></tr></thead><tbody className="divide-y divide-white/5">{history.map(item => { const manual = ['MANUAL_ADD','MANUAL_DEDUCT'].includes(item.transactionType); return <tr key={item.historyId}><td className="px-5 py-4 font-mono text-brand-orange">#{item.historyId}</td><td className="px-5 py-4 font-bold">{item.transactionType}</td><td className="px-5 py-4">{Number(item.pointChange) > 0 ? '+' : ''}{n(item.pointChange)}</td><td className="max-w-sm px-5 py-4 text-zinc-500">{item.caseId || item.reason || '—'}</td><td className="px-5 py-4 text-right">{manual ? <button type="button" onClick={() => { setTab('reverse'); setReverse(v => ({ ...v, historyId: String(item.historyId) })); }} className="text-xs font-black text-red-300 hover:text-red-200">Chọn để đảo</button> : null}</td></tr>; })}</tbody></table></div> : <p className="p-8 text-center text-sm text-zinc-500">Chưa có giao dịch.</p>}</article>
      </> : null}
    </section>
  );
}
