...
import { dateTime, leaveStatus, leaveType, shiftStatus } from '../employeePresentation';
...
export default function EmployeeSchedulePage() {
  ...
  const canCreateLeave = can(EMPLOYEE_PERMISSIONS.LEAVE_CREATE);
  ...
  return (
    <section className="space-y-6 text-white">
      <header className="flex flex-col gap-4 border-b border-zinc-800 pb-6 md:flex-row md:items-end md:justify-between">
        <div><p className="text-xs font-black uppercase tracking-[0.2em] text-amber-500">Lịch cá nhân</p><h1 className="mt-2 text-3xl font-black">Lịch làm & nghỉ phép</h1><p className="mt-2 text-sm text-zinc-500">Xem ca quản lý đã phân công và theo dõi các yêu cầu nghỉ của bạn.</p></div>
        <div className="flex gap-2"><input aria-label="Tháng làm việc" type="month" value={month} onChange={event => setMonth(event.target.value)} className="rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-2 text-sm" />{canCreateLeave && <button type="button" onClick={() => setLeaveOpen(true)} className="flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2 text-sm font-black text-black"><Plus size={16} /> Xin nghỉ</button>}</div>
      </header>
