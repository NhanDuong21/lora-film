...
import {
  ...
  LoaderCircle, MapPin, QrCode, RefreshCw, ShieldAlert, Ticket, Users, WalletCards,
} from 'lucide-react';
...
import { auditoriumLabel, clock, entryStatus, money, seatCount, shiftStatus } from '../employeePresentation';
...
import { getTicketCheckerSummary } from '../services/employeeTicketCheckerService';
...
export default function EmployeeDashboardPage() {
  ...
  const canScanTickets = can(EMPLOYEE_PERMISSIONS.TICKET_SCAN);
  const [data, setData] = useState({
    context: null, shifts: [], attendance: [], orders: [], cashSession: null, scanSummary: null,
  });
  ...
  const load = useCallback(async () => {
    ...
    const calls = [
      ...
      canScanTickets ? getTicketCheckerSummary(today) : Promise.resolve(null),
    ];
    const [context, shifts, attendance, orders, cashSession, scanSummary] = await Promise.allSettled(calls);
    setData({
      ...
      scanSummary: scanSummary.status === 'fulfilled' ? scanSummary.value : null,
    });
    const failed = [context, shifts, attendance, orders, cashSession, scanSummary]
      .filter(result => result.status === 'rejected').length;
    setState({
      loading: false,
      warning: failed ? 'Một vài số liệu chưa tải được. Bạn vẫn có thể tiếp tục các công việc đang hiển thị.' : '',
    });
  }, [canOperateCounter, canScanTickets, canViewAttendance, canViewSchedule]);
  ...
  return (
    <section className="mx-auto w-full max-w-7xl space-y-6 text-white">
        ...
        <div className="mt-2 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-400">
              {canScanTickets
                ? 'Xem ca làm, suất đang đón khách và kết quả soát vé của chính bạn.'
      ...
      {canScanTickets ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ['Lượt quét hôm nay', data.scanSummary?.totalScans || 0, QrCode, 'text-sky-300'],
          ['Khách đã vào', data.scanSummary?.admittedScans || 0, CheckCircle2, 'text-emerald-300'],
          ['Lượt bị từ chối', data.scanSummary?.rejectedScans || 0, ShieldAlert, 'text-red-300'],
          ['Khách còn chưa vào', data.scanSummary?.remainingTickets || 0, Users, 'text-amber-300'],
        ].map(([label, value, Icon, tone]) => <article key={label} className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-5"><div className="flex items-center justify-between"><p className="text-xs font-bold text-zinc-500">{label}</p><Icon size={18} className={tone} /></div><p className={`mt-3 text-2xl font-black ${tone}`}>{value}</p></article>)}
      </div> : null}
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        {canScanTickets ? <article className="rounded-3xl border border-amber-500/25 bg-amber-500/[0.04] p-6"><div className="flex items-center gap-2"><QrCode className="text-amber-400" size={20} /><h2 className="font-black">Cửa soát vé hôm nay</h2></div>{data.scanSummary?.showtimes?.length ? <div className="mt-4 space-y-3">{data.scanSummary.showtimes.slice(0, 3).map(showtime => { const status = entryStatus(showtime.entryStatus); return <div key={`${showtime.showtimePublicId}-${showtime.showtimeStart}`} className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-black">{showtime.movieTitle}</p><p className="mt-1 text-xs text-zinc-500">{clock(showtime.showtimeStart)} · {auditoriumLabel(showtime.auditoriumName)}</p></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${status.tone === 'emerald' ? 'bg-emerald-500/10 text-emerald-300' : status.tone === 'amber' ? 'bg-amber-500/10 text-amber-300' : 'bg-zinc-800 text-zinc-400'}`}>{status.label}</span></div><p className="mt-3 text-xs text-zinc-500">Đã vào <strong className="text-emerald-300">{showtime.admittedTickets}</strong> / {showtime.totalTickets} vé</p></div>; })}</div> : <div className="py-8 text-center"><Ticket className="mx-auto text-zinc-700" size={34} /><p className="mt-3 text-sm font-black text-zinc-400">Chưa có vé cần soát hôm nay</p></div>}<button type="button" onClick={() => navigate('/employee/ticket-scan')} className="mt-4 w-full rounded-xl bg-amber-500 py-3 text-sm font-black text-black">Mở màn hình soát vé</button></article> : null}
      </div>
      ...
      {canScanTickets ? <section className="rounded-3xl border border-zinc-800 bg-zinc-900/60 p-6"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><h2 className="font-black">Việc cần làm tiếp theo</h2><p className="mt-1 text-sm text-zinc-500">{activeAttendance ? 'Bạn đang trong ca. Chỉ cho khách vào khi kết quả quét hiển thị màu xanh.' : 'Kiểm tra ca làm và chấm công trước khi bắt đầu mở cửa phòng.'}</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => navigate('/employee/ticket-scan')} className="rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-black text-black">Bắt đầu soát vé</button><button type="button" onClick={() => navigate('/employee/ticket-showtimes')} className="rounded-xl border border-zinc-700 px-4 py-2.5 text-sm font-black">Xem suất chiếu</button><button type="button" onClick={load} className="flex items-center gap-2 rounded-xl border border-zinc-700 px-4 py-2.5 text-sm font-black"><RefreshCw size={16} /> Làm mới</button></div></div></section> : null}
    </section>
  );
}
