  ...
  return (
    <section className="flex-1 space-y-6 overflow-auto bg-zinc-950 p-6 text-white md:p-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-black uppercase">Quản lý Phòng ban</h1>
          <p className="mt-1 text-sm text-zinc-500">Danh sách các phòng ban trong hệ thống.</p>
        </div>
      ...
      <AsyncState loading={state.loading} error={state.error} onRetry={load} empty={!departments.length} emptyMessage="Chưa có phòng ban nào">
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-zinc-800">
              {departments.map(dept => (
                <tr key={dept.id}>
                  <td className="p-4 font-bold">{dept.code}</td>
                  <td className="p-4">{dept.name}</td>
                  <td className="p-4 text-zinc-400">{dept.description}</td>
                  <td className="space-x-3 p-4 text-right">
                    <button onClick={() => openModal(dept)} className="text-xs font-bold text-amber-400 hover:underline">Sửa</button>
                    <button onClick={() => handleDelete(dept.id)} className="text-xs font-bold text-red-400 hover:underline">Xóa</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <form onSubmit={handleSave} className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900 p-6 space-y-4">
            <h2 className="text-xl font-bold">{editingDept ? 'Sửa phòng ban' : 'Thêm phòng ban'}</h2>
            <Input required placeholder="Mã phòng ban" value={formData.code} onChange={e => setFormData({ ...formData, code: e.target.value })} />
            <Input required placeholder="Tên phòng ban" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
